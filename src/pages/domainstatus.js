...
const DomainStatus = () => {
  ...
  const fetchDomainInfo = async () => {
    ...
    try {
      const response = await fetch(`https://www.whoisxmlapi.com/whoisserver/WhoisService?apiKey=key&domainName=${domainName}&outputFormat=json`);
      if (!response.ok) {
      ...
      // Additional logic to clear domain info if no registrant information exists
      if (!data.WhoisRecord?.registryData?.registrant) {
        setNotification('Domain is available for purchase!');
        setDomainInfo(null);
      }
    } catch (error) {
